...
if(trim($expression) == '')
{
    echo "<div class='title'><h1>Search by typing a word or phrase in the search box.</h1></div>";
}else{

    ?>
    <div class="title-title">
        <h1>Result for: &nbsp;<?php echo $expression;  ?> </h1>
    </div>

    <div class="main">

<?php
$apiSearch = file_get_contents("https://imdb-api.com/en/API/SearchTitle/k_24d933uc/" . $expression);
//$apiSearch=file_get_contents("https://imdb-api.com/en/API/SearchTitle/k_24d933uc/dune");
$apiSearch = json_decode($apiSearch, true);
foreach ($apiSearch['results'] as $result) {
    $id = $result['id'];
    $image = $result['image'];
    $title = $result['title'];
